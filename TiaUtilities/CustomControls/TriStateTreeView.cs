...
namespace TiaUtilities.CustomControls
{
    ...
    public class TriStateTreeView : TreeView
    {
        ...
        public TriStateStyles TriStateStyleProperty { get => TriStateStyle; set => TriStateStyle = value; }
        ...
        public void UpdateAllNodesState()
        {
            foreach (TreeNode treeNode in base.Nodes)
            {
                this.UpdateParentState(treeNode);
                this.UpdateChildState(treeNode.Nodes, treeNode.StateImageIndex, treeNode.Checked, true);
            }
        }
        ...
        protected override void OnAfterCheck(TreeViewEventArgs e)
        {
            ...
            UpdateChildState(treeNode.Nodes, treeNode.StateImageIndex, treeNode.Checked, false);  // force all children to inherit the same state as the current node
            UpdateParentState(treeNode.Parent); // populate state up the tree, possibly resulting in parents with mixed state
        ...
        protected override void OnAfterExpand(TreeViewEventArgs e)
        {
            ...
            if (treeNode != null)
            {
                UpdateChildState(treeNode.Nodes, treeNode.StateImageIndex, treeNode.Checked, true);
            }
        ...
        protected void UpdateChildState(TreeNodeCollection Nodes, int StateImageIndex, bool Checked, bool ChangeUninitialisedNodesOnly)
        {
            foreach (TreeNode treeNode in Nodes)
            {
                if (!ChangeUninitialisedNodesOnly || treeNode.StateImageIndex == -1)
                {
                    ...
                    treeNode.Checked = Checked;  // override 'checked' state of child with that of parent

                    if (treeNode.Nodes.Count > 0)
                    {
                        UpdateChildState(treeNode.Nodes, StateImageIndex, Checked, ChangeUninitialisedNodesOnly);
                    }
                }
            }
        }
        ...
        protected void UpdateParentState(TreeNode treeNode)
        {
            ...
            int OrigStateImageIndex = treeNode.StateImageIndex;
            ...
            {
                if (treeNodeChild.StateImageIndex == (int)CheckedState.Checked)
                {
                    CheckedNodes++;
                }
                else if (treeNodeChild.StateImageIndex == (int)CheckedState.Mixed)
                {
                    MixedNodes++;
                    break;
                }
            ...
            if (TriStateStyle == TriStateStyles.Installer)
            {
                ...
                if (MixedNodes == 0)
                {
                    if (UnCheckedNodes == 0)
                    {
                        treeNode.Checked = true; // all children are checked, so parent must be checked
                    }
                    else
                    {
                        treeNode.Checked = false; // at least one child is unchecked, so parent must be unchecked
                    }
                }
            }
